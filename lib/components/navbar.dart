import 'package:jaspr/dom.dart';
import 'package:jaspr/jaspr.dart';

import '../constants/data.dart';

class Navbar extends StatelessComponent {
  const Navbar({super.key});

  @override
  Component build(BuildContext context) {
    return nav(classes: 'navbar', [
      // Logo
      a(
        href: '#hero',
        classes: 'nav-logo',
        [
          Component.text(PersonalInfo.firstName),
          span(classes: 'accent', [Component.text('.dev')]),
        ],
      ),

      // Desktop nav links
      div(classes: 'nav-links', [
        a(href: '#about', [Component.text('About')]),
        a(href: '#experience', [Component.text('Experience')]),
        a(href: '#projects', [Component.text('Projects')]),
        a(href: '#contact', classes: 'btn-nav', [Component.text('Get in touch')]),
      ]),

      // Mobile menu button
      button(
        id: 'mobile-menu-btn',
        classes: 'mobile-menu-btn',
        [
          // Hamburger icon
          RawText('''
            <svg id="menu-icon-open" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
              viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
              stroke-linecap="round" stroke-linejoin="round">
              <line x1="3" y1="12" x2="21" y2="12"></line>
              <line x1="3" y1="6" x2="21" y2="6"></line>
              <line x1="3" y1="18" x2="21" y2="18"></line>
            </svg>
            <svg id="menu-icon-close" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
              viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
              stroke-linecap="round" stroke-linejoin="round" style="display:none">
              <line x1="18" y1="6" x2="6" y2="18"></line>
              <line x1="6" y1="6" x2="18" y2="18"></line>
            </svg>
          '''),
        ],
      ),
    ]);
  }
}
